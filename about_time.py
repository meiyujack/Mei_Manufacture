...
class HumanBeing:
    ...
    SIGN = {
        ("0321 0721", "0420 1817"): [
            {"星宫": "白羊", "拉丁": "Aries", "角度": (0, 30)},
            {"春分": 0, "清明": 15},
        ],
        ("0420 1817", "0521 1721"): [
            {"星宫": "金牛", "拉丁": "Taurus", "角度": (30, 60)},
            {"谷雨": 30, "立夏": 45},
        ],
        ("0521 1721", "0622 0116"): [
            {"星宫": "双子", "拉丁": "Gemini", "角度": (60, 90)},
            {"小满": 60, "芒种": 75},
        ],
        ("0622 0116", "0723 1212"): [
            {"星宫": "巨蟹", "拉丁": "Cancer", "角度": (90, 120)},
            {"夏至": 90, "小暑": 105},
        ],
        ("0723 1212", "0823 1921"): [
            {"星宫": "狮子", "拉丁": "Leo", "角度": (120, 150)},
            {"大暑": 120, "立秋": 135},
        ],
        ("0823 1921", "0923 1705"): [
            {"星宫": "室女", "拉丁": "Virgo", "角度": (150, 180)},
            {"处属": 150, "白露": 165},
        ],
        ("0923 1705", "1024 0230"): [
            {"星宫": "天秤", "拉丁": "Libra", "角度": (180, 210)},
            {"秋分": 180, "寒露": 195},
        ],
        ("1024 0230", "1123 0008"): [
            {"星宫": "天蝎", "拉丁": "Scorpio", "角度": (210, 240)},
            {"霜降": 210, "立冬": 225},
        ],
        ("1123 0008", "1222 1330"): [
            {"星宫": "射手", "拉丁": "Sagittarius", "角度": (240, 270)},
            {"小雪": 240, "大雪": 255},
        ],
        ("1222 1330", "0121 0010"): [
            {"星宫": "摩羯", "拉丁": "Capricorn", "角度": (270, 300)},
            {"冬至": 270, "小寒": 285},
        ],
        ("0121 0010", "0219 1418"): [
            {"星宫": "宝瓶", "拉丁": "Aquarius", "角度": (300, 330)},
            {"大寒": 300, "立春": 315},
        ],
        ("0219 1418", "0320 1314"): [
            {"星宫": "双鱼", "拉丁": "Pisces", "角度": (330, 0)},
            {"雨水": 330, "惊蛰": 345},
        ],
    }
    CENTURY_21_C = {
    ...
    def get_JIE_QI(self):
        ...
        near_jieqi = list(
            HumanBeing.get_sign(datetime.now() + DT.timedelta(days=15))[1].keys()
        )
        d = [
            int(round(y * 0.2422 + HumanBeing.CENTURY_21_C[k] - int(y / 4), 1))
            for k in near_jieqi
        ]
        r = dict(zip(near_jieqi, d))
        if int(datetime.now().strftime("%d")) > 15:
            print(
                f'即将来临的节气是{list(r.keys())[1]},为{int(datetime.now().strftime("%m"))+1 if int(datetime.now().strftime("%m"))+1 <=12 else int(datetime.now().strftime("%m"))+1-12}月{list(r.values())[1]}号'
            )
        else:
            print(
    ...
    def __init__(self, birth: str):
        ...
        try:
            ...
            raise "格式错误"
        self.gan_zhi = (
            "".join(HumanBeing.TIAN_GAN)[(self.birth.year - HumanBeing.INIT) % 10]
    ...
    def get_sign(dt: datetime):
        for s, v in HumanBeing.SIGN.items():
            if "1222" <= dt.strftime("%m%d") or dt.strftime("%m%d") < "0121":
                return [
                    {"星宫": "摩羯", "拉丁": "Capricorn", "角度": (270, 300)},
                    {"冬至": 270, "小寒": 285},
                ]
    ...
    def update_feb(year):
        HumanBeing.DAYS.update({2: 29}) if HumanBeing.is_leap_year(
            year
        ) else HumanBeing.DAYS.update({2: 28})
        HumanBeing.DAYS = {k: HumanBeing.DAYS[k] for k in sorted(HumanBeing.DAYS)}

    def get_days(self):
        _sum = 0
        r = [
            HumanBeing.is_leap_year(y)
            for y in range(self.birth.year + 1, datetime.now().year)
        ]
